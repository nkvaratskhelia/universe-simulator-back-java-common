...
=== Lifecycle policies
Go to `Stack Management > Index Management > Index Lifecycle Policies`
and add `zipkin-span_template` and `logstash` to the `30-days-default`
policy.

=== Index patterns
Go to `Stack Management > Index Management > Index Patterns` and add
the following index pattern:
